...
import { currentUser, can } from '../lib/auth.js'
...
export default function Team() {
  ...
  const allowed = can('cap:manage_users')
  ...
  useEffect(() => { load(); api.get('/api/roles').then(setRoles).catch(() => {}) }, [])

  const counts = useMemo(() => {
    const byRole = {}
    users.forEach((u) => { byRole[u.role] = (byRole[u.role] || 0) + 1 })
    return { total: users.length, roles: roles.length, admins: byRole.admin || 0, byRole }
  }, [users, roles])
  ...
  const changeRole = async (u, role) => {
    ...
    try { await api.patch(`/api/users/${u.id}`, { role }); toast(`${u.name} → ${roleLabel(role)}`, 'success') }
      ...
      <div className="flex h-screen flex-col overflow-hidden">
        <header className="flex h-16 shrink-0 items-center justify-between gap-2 border-b border-slate-200 bg-white px-3 sm:px-6">
          <div className="flex min-w-0 shrink items-center gap-3 text-slate-700">
            ...
            <div><h1 className="truncate text-lg font-bold leading-tight">Users</h1><p className="hidden truncate text-[11px] text-slate-500 sm:block">Manage users &amp; their roles.</p></div>
          </div>
          <div className="flex items-center gap-3">
            {allowed && <Link to="/roles" className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold hover:bg-slate-50">Roles &amp; Permissions</Link>}
            {allowed && <button onClick={() => setShowAdd((s) => !s)} className="rounded-lg bg-brand-600 px-3 py-2 text-sm font-semibold text-white hover:bg-brand-700">+ Add User</button>}
            <TopBarUser />
          </div>
        </header>
        ...
        <main className="nice-scroll flex-1 overflow-y-auto px-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs text-slate-500">Total Users</div><div className="mt-1 text-2xl font-extrabold">{counts.total}</div></div>
            <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs text-slate-500">Roles</div><div className="mt-1 text-2xl font-extrabold">{counts.roles}</div></div>
            <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs text-slate-500">Admins</div><div className="mt-1 text-2xl font-extrabold">{counts.admins}</div></div>
          </div>

          {showAdd && (
            <form onSubmit={addMember} className="mt-5 rounded-2xl border border-slate-200 bg-white p-5">
              <h3 className="mb-3 text-sm font-bold">Add User</h3>
              ...
              <div className="mt-3 flex gap-2">
                <button type="submit" disabled={busy} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-60">{busy ? 'Adding…' : 'Add User'}</button>
          ...
          <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table className="w-full min-w-[640px] text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr>
                <th className="px-3 py-3 text-left">User</th><th className="px-3 py-3 text-left">Email</th><th className="px-3 py-3 text-left">Role</th><th className="px-3 py-3 text-left">Joined</th><th className="px-3 py-3 text-right">Actions</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-100">
                {users.length === 0 && <tr><td colSpan="5" className="px-3 py-10 text-center text-sm text-slate-400">{loading ? 'Loading…' : 'No users yet'}</td></tr>}
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3">
                      <select value={u.role} onChange={(e) => changeRole(u, e.target.value)} className={`rounded-md px-2 py-1 text-xs font-semibold ${ROLE_CLS[u.role] || 'bg-slate-100 text-slate-600'}`}>
                        {roles.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
                      </select>
                    </td>
