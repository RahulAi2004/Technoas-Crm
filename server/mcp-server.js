...
server.tool('get_chat', 'The FULL chat transcript with a customer (exact words), oldest→newest. By default returns the whole conversation.',
  { name: z.string().describe('Customer name (partial ok)'), limit: z.number().optional().default(3000).describe('Max messages (default 3000 = the whole chat; biggest chat is ~2421). Pass a smaller number for just the recent tail.') },
  async ({ name, limit }) => {
    ...
    const r = await q(`SELECT direction, message_type, body, to_char(COALESCE(sent_at, created_at),'YYYY-MM-DD HH24:MI') ts
       ...
       ORDER BY COALESCE(sent_at, created_at) DESC LIMIT $2`, [c.customer_id, Math.min(limit || 3000, 5000)])
