...
const APP_MAP = {
  ...
  leads:         { scalar: (d) => ({ stage: d.pipeline || 'new_inquiry', status: d.status || 'New', source: d.source || null, score_total: Number(d.score) || 0, estimated_value: Number(d.value) || 0 }), fk: (d) => ({ customer_id: ['customers', d.conversation_id ? `cust:${d.conversation_id}` : null], conversation_id: ['conversations', d.conversation_id] }) },
